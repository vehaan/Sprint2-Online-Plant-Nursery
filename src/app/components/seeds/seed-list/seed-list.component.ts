...
export class SeedListComponent implements OnInit {
  ...
  ngDoCheck(): void {
    ...
    if (
      ...
      let result = tempSeeds.filter((o) =>
        bloomSeeds.some(({ id, name }) => o.id === id && o.name === name)
      );
  ...
  onEdit(seed: Seed) {
    this.router.navigate(['edit-seed', seed.id]);
  }

  onDelete(seed: Seed): void {
    this.service.deleteSeedById(seed.id).subscribe((data) => {
